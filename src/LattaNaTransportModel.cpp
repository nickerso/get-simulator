...
LattaNaTransportModel::LattaNaTransportModel()
{
	const int N = 5;
	C_a.resize(N);
	C_b.resize(N);
...
void LattaNaTransportModel::initialise()
{
...
//    std::cout << "hack salt-stepper!" << std::endl;
//    C_a[Na] = C_b[Na] = 0.0;
//    C_a[Cl] = C_b[Cl] = 0.0;
...
double LattaNaTransportModel::solveForElectroneutrality(int& errorFlag, const bool updateOnly)
{
    errorFlag = 0;
	/**
...
double LattaNaTransportModel::solveForElectroneutralityVoltageClamp(const bool updateOnly)
{
	/**
...
void LattaNaTransportModel::calculateSoluteMembraneFluxes()
{
    int i, N=J_a.size();
	/*
	...
	/*
	 * Basolateral membrane flux also passive + an active Na-K pump (eq 9)
	 */
    if (debugLevel() > 99) std::cout << "basolateral membrane fluxes";
    calculatePassiveFluxes(J_b, P_b, z, C_c, C_b, U_b);
    for (i=0; i<N; i++) J_p[i] = 0.0;
	double I_p = Imax / ((1.0 + pow(K_Na/C_c[Na], n_Na)) * (1.0 + pow(K_K/C_b[K], n_K)));
	J_p[Na] = n_Na * I_p / F;
    J_p[K] = -n_K * I_p / F;
    for (i=0; i<N; i++) J_b[i] += J_p[i];
}

void LattaNaTransportModel::calculateSoluteParacellularFluxes()
{
    /*
...
void LattaNaTransportModel::compute_I_a()
{
	I_a = 0;
...
void LattaNaTransportModel::compute_I_b()
{
    I_b = 0;
...
void LattaNaTransportModel::compute_I_j()
{
    I_j = 0;
...
void LattaNaTransportModel::calculatePassiveFluxes(std::vector<double>& J, const std::vector<double>& P,
...
void LattaNaTransportModel::printState(std::ostream& s, double &time)
{
    s << time << "\t" << V;
    E_t = calcE(U_t);
    E_a = calcE(U_a);
...
void LattaNaTransportModel::printStateHeader(std::ostream& s)
{
    s << "# time";          // 1
    s << "\tV";             // 2
    s << "\tE_t";           // 3
...
std::vector<double> LattaNaTransportModel::calculateRHS(double time, int &errorFlag)
{
...
void LattaNaTransportModel::calculateWaterFluxes()
{
    Jw_a = Jw_b = 0;
    for (int i = 0; i < C_c.size(); ++i)
    {
...
void LattaNaTransportModel::initialiseSaltStepper()
{
