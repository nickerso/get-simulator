...
static std::vector<double> Evaluate(libsbml::ASTNode tree,
                                    ...
                                    const ParameterList& parameters)
{
    ...
    {
        const Variable& variable = variables[tree.getName()];
...
static double EvaluateSingle(ASTNode node, int index,
                             const VariableList& variables,
                             const ParameterList& parameters)
{
    ...
    if (node.isOperator())
    {
        ...
        case libsbml::AST_MINUS:
            if (node.getNumChildren() == 1)
                return -1.0 * EvaluateSingle(node.getLeftChild(), index,
    ...
    else if (node.isName())
    {
        ...
        {
            const Variable& variable = variables[node.getName()];
    ...
    case libsbml::AST_FUNCTION_ABS:
        return
          Math.Abs(EvaluateSingle(node.getLeftChild(), index, sVariableIds, oVariableData,
                                  ListOfParameters));
